export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "DELETE" && url.pathname === "/delete") {
      ...
      await env.GUEST_DATA.delete(id);
    ...
    if (request.method === "GET" && url.pathname === "/admin") {
      ...
      for (const {name} of list.keys) {
        const v = await env.GUEST_DATA.get(name);
...
<h2>宾客回复 (${rows.length}条) <span style="font-size:13px;color:#999;font-weight:normal">点击删除按钮删除记录</span></h2>
<table><tr><th>姓名</th><th>出席</th><th>人数</th><th>留言</th><th>时间</th><th>操作</th></tr>
${rows.map(r => `<tr id="row-${esc(r.id)}"><td>${esc(r.name)}</td><td>${r.attending==="yes"?"✅ 是":"❌ 否"}</td><td>${esc(r.guests)}</td><td>${esc(r.message)}</td><td>${esc(r.time)}</td><td><button class="del-btn" onclick="del('${esc(r.id)}')">删除</button></td></tr>`).join("")}
</table>
